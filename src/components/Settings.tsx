...
import { User, Shield, Key, AlertTriangle, RefreshCw, Trash2, CheckCircle2, UserX, Save } from 'lucide-react';
...
export const SettingsTab: React.FC<SettingsProps> = ({ userProfile, session, onRefreshData }) => {
  ...
  const handleApproveDeletion = async (id: string, name: string) => {
    ...
    try {
      // Delete from auth (using admin helper if we have it, otherwise inform user we can't delete auth natively from client)
      // Actually we will use supabaseAdminAuth
